...
export default (option, Dayex, dayex) => {
  ...
  proto.utc = function (keepLocalTime) {
    const ins = dayex(this.toDate(), { locale: this.$L, utc: true })
    if (keepLocalTime) {
  ...
  proto.local = function () {
    return dayex(this.toDate(), { locale: this.$L, utc: false })
  }
  ...
  proto.toDate = function (type) {
    if (type === 's' && this.$offset) {
      return dayex(this.format('YYYY-MM-DD HH:mm:ss:SSS')).toDate()
    }
  ...
  proto.diff = function (input, units, float) {
    ...
    const localInput = dayex(input).local()
