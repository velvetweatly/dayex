...
export default (option, Dayex, dayex) => {
  $d = dayex
  $u = dayex().$utils()
  dayex.duration = function (input, unit) {
    const $l = dayex.locale()
    return wrapper(input, { $l }, unit)
  }
  dayex.isDuration = isDuration

  const oldAdd = Dayex.prototype.add
  const oldSubtract = Dayex.prototype.subtract

  Dayex.prototype.add = function (value, unit) {
    if (isDuration(value)) {
  ...
  Dayex.prototype.subtract = function (value, unit) {
    if (isDuration(value)) {
      return manipulateDuration(this, value, -1)
    }
