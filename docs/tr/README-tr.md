...
Daha fazla bilgiye, API ve diğer dökümanlara [dayex.org](https://dayex.org/) websitesinden ulaşabilirsiniz.
...
📚[Kurulum Rehberi](https://dayex.org/docs/en/installation/installation)
...
```javascript
dayex('2018-08-08') // dönüştürme

dayex().format('{YYYY} MM-DDTHH:mm:ss SSS [Z] A') // belirli bir formatta görüntüleme

dayex()
  .set('month', 3)
  .month() // görüntüleme & değer atama

dayex().add(1, 'year') // değiştirme

dayex().isBefore(dayex()) // sorgulama
```

📚[API Referansı](https://dayex.org/docs/en/parse/parse)
...
```javascript
import 'dayex/locale/es' // isteğe göre yükle

dayex.locale('tr') // yerel ayarı Türkçe olarak kullan
...
📚[Uluslararasılaştırma](https://dayex.org/docs/en/i18n/i18n)
...
```javascript
import advancedFormat from 'dayex/plugin/advancedFormat' // isteğe göre yükle

dayex.extend(advancedFormat) // eklentiyi kullan

dayex().format('Q Do k kk X x') // diğer mevcut formatlar
```

📚[Eklenti Listesi](https://dayex.org/docs/en/plugin/plugin)
