...
詳細、API、その他のドキュメントについては、[dayex.org](https://dayex.org/) のウェブサイトを参照してください。
...
📚[インストールガイド](https://dayex.org/docs/en/installation/installation)
...
```javascript
dayex('2018-08-08') // パース

dayex().format('{YYYY} MM-DDTHH:mm:ss SSS [Z] A') // 表示
...
dayex().isBefore(dayex()) // クエリ
```

📚[API リファレンス](https://dayex.org/docs/en/parse/parse)
...
```javascript
import 'dayex/locale/es' // 必要に応じて読み込み

dayex.locale('es') // スペイン語をグローバルロケールとして適用
...
📚[国際化](https://dayex.org/docs/en/i18n/i18n)
...
```javascript
import advancedFormat from 'dayex/plugin/advancedFormat' // 必要に応じて読み込み

dayex.extend(advancedFormat) // プラグインを使用

dayex().format('Q Do k kk X x') // 多様なフォーマットが利用可能に
```

📚[プラグインリスト](https://dayex.org/docs/en/plugin/plugin)
