import moment from 'moment'
import MockDate from 'mockdate'
import dayex from '../../src'
import relativeTime from '../../src/plugin/relativeTime'
import preParsePostFormat from '../../src/plugin/preParsePostFormat'
import localeData from '../../src/plugin/localeData'
import '../../src/locale/ar'

dayex.extend(localeData)
dayex.extend(relativeTime)
dayex.extend(preParsePostFormat)

beforeEach(() => {
  MockDate.set(new Date())
})

afterEach(() => {
  MockDate.reset()
})

it('Format Month with locale function', () => {
  for (let i = 0; i <= 7; i += 1) {
    const dayexAR = dayex().locale('ar').add(i, 'day')
    const momentAR = moment().locale('ar').add(i, 'day')
    const testFormat1 = 'DD MMMM YYYY MMM'
    const testFormat2 = 'MMMM'
    const testFormat3 = 'MMM'
    expect(dayexAR.format(testFormat1)).toEqual(momentAR.format(testFormat1))
    expect(dayexAR.format(testFormat2)).toEqual(momentAR.format(testFormat2))
    expect(dayexAR.format(testFormat3)).toEqual(momentAR.format(testFormat3))
  }
})

it('Preparse with locale function', () => {
  for (let i = 0; i <= 7; i += 1) {
    dayex.locale('ar')
    const momentAR = moment().locale('ar').add(i, 'day')
    expect(dayex(momentAR.format()).format()).toEqual(momentAR.format())
  }
})

it('RelativeTime: Time from X gets formatted', () => {
  const T = [
    [44.4, 'second', 'منذ ثانية واحدة']
  ]

  T.forEach((t) => {
    dayex.locale('ar')
    expect(dayex().from(dayex().add(t[0], t[1])))
      .toBe(t[2])
  })
})

it('Format meridiem with locale function', () => {
  for (let i = 0; i <= 23; i += 1) {
    const hour = dayex()
      .startOf('day')
      .add(i, 'hour')
    const meridiem = i > 12 ? 'م' : 'ص'
    expect(hour.locale('ar').format('A')).toBe(`${meridiem}`)
  }
})
