...
describe('Parse', () => {
  ...
  it('moment-js like formatted dates', () => {
    ...
    let d = '20130108'
    expect(dayex(d).valueOf()).toBe(moment(d).valueOf())
    d = '2018-04-24'
    expect(dayex(d).valueOf()).toBe(moment(d).valueOf())
    d = '2018-04-24 11:12'
    expect(dayex(d).format()).toBe(moment(d).format()) // not recommend
    d = '2018-05-02 11:12:13'
    expect(dayex(d).valueOf()).toBe(moment(d).valueOf())
    d = '2018-05-02 11:12:13.998'
    expect(dayex(d).valueOf()).toBe(moment(d).valueOf())
    d = '2018-4-1'
    expect(dayex(d).valueOf()).toBe(moment(d).valueOf()) // not recommend
    d = '2018-4-1 11:12'
    expect(dayex(d).format()).toBe(moment(d).format()) // not recommend
  ...
  it('String ISO 8601 date, time and zone', () => {
    ...
    expect(dayex(time).valueOf()).toBe(moment(time).valueOf())
  })
  ...
  it('rejects invalid values', () => {
    expect(dayex({}).isValid()).toBe(false)
    expect(dayex(() => '2018-01-01').isValid()).toBe(false)
    expect(dayex(Infinity).isValid()).toBe(false)
    expect(dayex(NaN).isValid()).toBe(false)
    expect(dayex([2018, 5, 1, 13, 52, 44]).isValid()).toBe(false) // Arrays with time part
  })

  it('parses Arrays with date part', () => {
    const dateParts = [2018, 5, 1]
    const expected = '2018-05-01T00:00:00.000Z'
    const d = dayex(dateParts)
  ...
  it('String Other, Undefined and Null and isValid', () => {
    ...
    expect(dayex('otherString').toString().toLowerCase()).toBe(moment('otherString').toString().toLowerCase())
    expect(dayex(undefined).toDate()).toEqual(moment(undefined).toDate())
    expect(dayex().isValid()).toBe(true)
    expect(dayex(undefined).isValid()).toBe(true)
    expect(dayex('').isValid()).toBe(false)
    expect(dayex(null).isValid()).toBe(false)
    expect(dayex('otherString').isValid()).toBe(false)
    expect(dayex(null).toString().toLowerCase()).toBe(moment(null).toString().toLowerCase())
  })
})

it('Unix Timestamp Number (milliseconds) 1523520536000', () => {
  const timestamp = 1523520536000
  expect(dayex(timestamp).valueOf()).toBe(moment(timestamp).valueOf())
})

it('Unix Timestamp Number (seconds) 1318781876', () => {
  const timestamp1 = 1318781876
  const timestamp2 = 1318781876.721
  expect(dayex.unix(timestamp1).valueOf()).toBe(moment.unix(timestamp1).valueOf())
  expect(dayex.unix(timestamp2).valueOf()).toBe(moment.unix(timestamp2).valueOf())
})

it('String and Number 20180101', () => {
  expect(dayex(20180101).valueOf()).toBe(moment(20180101).valueOf())
  expect(dayex('20180101').valueOf()).toBe(moment('20180101').valueOf())
})

it('Number 0', () => {
  expect(dayex(0).valueOf()).toBe(moment(0).valueOf())
})

it('Clone not affect each other', () => {
  const base = dayex(20170101)
...
describe('REGEX_PARSE', () => {
  it('2020/9/30', () => {
    ...
    expect(dayex(date).valueOf()).toBe(moment(date).valueOf())
