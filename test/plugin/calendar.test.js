...
it('No argument && null && undefined', () => {
  expect(dayex().calendar()).toEqual(moment().calendar())
...
it('Custom format', () => {
  ...
  expect(dayex().calendar(null, format)).toEqual(moment().calendar(null, format))
  const now = '2015-01-15T14:21:22.000Z'
  const nextDayWithoutFormat = '2015-01-14T11:23:55.000Z'
  expect(dayex(now).calendar(nextDayWithoutFormat, format))
...
it('Custom callback', () => {
  ...
  const now = '2015-01-15T14:21:22.000Z'
  const nextDayWithoutFormat = '2015-01-14T11:23:55.000Z'
  expect(dayex(now).calendar(nextDayWithoutFormat, callbacks))
...
it('callback is a function with the scope of the current moment', () => {
  ...
  expect(dayex().calendar(null, callbacks)).toEqual(callbacks.sameDay())
  const callbacks2 = {
    sameDay: function cb() {
      return this
    }
  }
  const result = dayex().calendar(null, callbacks2)
  expect(result.format).not.toBeUndefined()
  expect(dayex.isDayex(result)).toBeTruthy()
})
...
it('callback is a function and first argument a moment that depicts now', () => {
  ...
  const now = dayex()
  dayex(now).calendar(now, callbacks)
