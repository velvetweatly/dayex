...
describe('UTC Get', () => {
  it('UTC Year', () => {
    expect(dayex().utc().year()).toBe(moment().utc().year())
  })

  it('UTC Month', () => {
    expect(dayex().utc().month()).toBe(moment().utc().month())
  })

  it('UTC Day of Week', () => {
    expect(dayex().utc().day()).toBe(moment().utc().day())
  })

  it('UTC Date', () => {
    expect(dayex().utc().date()).toBe(moment().utc().date())
  })

  it('UTC Hour', () => {
    expect(dayex().utc().hour()).toBe(moment().utc().hour())
  })

  it('UTC Minute', () => {
    expect(dayex().utc().minute()).toBe(moment().utc().minute())
  })

  it('UTC Second', () => {
    expect(dayex().utc().second()).toBe(moment().utc().second())
  })

  it('UTC Millisecond', () => {
    expect(dayex().utc().millisecond()).toBe(moment().utc().millisecond())
  })
})

describe('Parse UTC ', () => {
  it('Parse Now', () => {
    expect(dayex.utc().format()).toBe(moment.utc().format())
    expect(dayex().utc().format()).toBe(moment().utc().format())
  })

  it('Parse date string without timezone', () => {
    const d = '2018-09-06'
    expect(dayex.utc(d).format()).toEqual(moment.utc(d).format())
    expect(dayex.utc(d).format()).toEqual('2018-09-06T00:00:00Z')
    expect(dayex(d).utc().format()).toEqual(moment(d).utc().format())
    const d2 = '2018-09'
    expect(dayex.utc(d2).format()).toEqual(moment.utc(d2).format())
    expect(dayex.utc(d2).format()).toEqual('2018-09-01T00:00:00Z')
    expect(dayex(d2).utc().format()).toEqual(moment(d2).utc().format())
    const d3 = '2018'
    expect(dayex.utc(d3).format()).toEqual(moment.utc(d3).format())
    expect(dayex.utc(d3).format()).toEqual('2018-01-01T00:00:00Z')
    expect(dayex(d3).utc().format()).toEqual(moment(d3).utc().format())
  })
  ...
  it('Parse date string without timezone', () => {
    ...
    expect(dayex.utc(d).format()).toEqual('2017-04-22T19:50:16Z')
    expect(dayex.utc(d).format()).toEqual(moment.utc(d).format())
    expect(dayex(d).utc().format()).toBe(moment(d).utc().format())
  ...
  it('Parse date string set utc in config', () => {
    ...
    expect(dayex(d, { utc: true }).format()).toEqual('2018-09-06T19:34:28Z')
    expect(dayex(d, { utc: true }).format()).toEqual(moment(d).utc().format())
    expect(dayex(d).utc().format()).toEqual('2018-09-06T19:34:28Z')
    expect(dayex(d).utc().format()).toEqual(moment(d).utc().format())
    expect(dayex.utc(d).format()).toEqual('2018-09-06T19:34:28Z')
    expect(dayex.utc(d).format()).toEqual(moment.utc(d).format())
  })

  it('parses unlimited millisecond in utc', () => {
    const date = '2019-03-25T06:41:00.999999999'
    const ds = dayex.utc(date)
...
it('Clone retains the UTC mode', () => {
  const instance = dayex('2018-09-06').utc()
...
describe('local', () => {
  it('Returns a new instance', () => {
    const instance = dayex.utc('2018-09-06T19:34:28.657Z')
  ...
  it('UTC to local', () => {
    const d = '2018-09-06'
    expect(dayex.utc(d).local().format()).toEqual(moment.utc(d).local().format())
  })
})
...
it('StartOf EndOf Year ... in UTC mode', () => {
  ...
  testArr.forEach((d) => {
    expect(dayex().utc().startOf(d).format()).toBe(moment().utc().startOf(d).format())
    expect(dayex().utc().endOf(d).format()).toBe(moment().utc().endOf(d).format())
  })
})

describe('UTC Set', () => {
  it('Set UTC Day', () => {
    expect(dayex().utc().set('date', 30).valueOf()).toBe(moment().utc().set('date', 30).valueOf())
  })

  it('Set UTC Day of Week', () => {
    expect(dayex().utc().set('day', 0).valueOf()).toBe(moment().utc().set('day', 0).valueOf())
  })

  it('Set UTC Month', () => {
    expect(dayex().utc().set('month', 11).valueOf()).toBe(moment().utc().set('month', 11).valueOf())
  })

  it('Set UTC Year', () => {
    expect(dayex().utc().set('year', 2008).valueOf()).toBe(moment().utc().set('year', 2008).valueOf())
  })

  it('Set UTC Hour', () => {
    expect(dayex().utc().set('hour', 6).valueOf()).toBe(moment().utc().set('hour', 6).valueOf())
  })

  it('Set UTC Minute', () => {
    expect(dayex().utc().set('minute', 59).valueOf()).toBe(moment().utc().set('minute', 59).valueOf())
  })

  it('Set UTC Second', () => {
    expect(dayex().utc().set('second', 59).valueOf()).toBe(moment().utc().set('second', 59).valueOf())
  })

  it('Set UTC Millisecond', () => {
    expect(dayex().utc().set('millisecond', 999).valueOf()).toBe(moment().utc().set('millisecond', 999).valueOf())
  })
})

it('isUTC', () => {
  expect(dayex().isUTC()).toBe(false)
  expect(dayex().utc().isUTC()).toBe(true)
  expect(dayex.utc().isUTC()).toBe(true)
})

describe('UTC and local', () => {
  const localDay = dayex(Date.UTC(2011, 1, 2, 3, 4, 5, 6))
  ...
  it('utcOffset', () => {
    ...
    expect(dayex().utc().utcOffset()).toBe(0)
  })
})
...
describe('UTC with customParseFormat', () => {
  it('Custom Parse Format', () => {
    ...
    const instant = dayex.utc('2011-02-02 03:04:05', 'YYYY-MM-DD HH:mm:ss')
...
describe('UTC Offset', () => {
  it('get utcOffset', () => {
    expect(dayex().utcOffset()).toBe(moment().utcOffset())
    expect(dayex().utc().utcOffset()).toBe(moment().utc().utcOffset())
  })

  it('get utc offset with a number value', () => {
    const time = '2021-02-28 19:40:10'
    const hoursOffset = -8
    const daysJS = dayex(time).utc().utcOffset(hoursOffset * 60, true)
  ...
  it('get utc offset with a negative valid string value, format: HH:mm', () => {
    const time = '2021-02-28 19:40:10'
    const hoursOffset = -8
    const daysJS = dayex(time).utc().utcOffset(`-0${Math.abs(hoursOffset)}:00`, true)
  ...
  it('get utc offset with a positive valid string value, format: HH:mm', () => {
    const time = '2021-02-28 19:40:10'
    const hoursOffset = 8
    const daysJS = dayex(time).utc().utcOffset(`+0${hoursOffset}:00`, true)
  ...
  it('get utc offset with an invalid string value, value: random', () => {
    const time = '2021-02-28 19:40:10'
    const daysJS = dayex(time, { utc: true }).utc(true).utcOffset('random')
  ...
  it('get utc offset with an invalid string value, value: 0', () => {
    const time = '2021-02-28 19:40:10'
    const daysJS = dayex(time, { utc: true }).utc(true).utcOffset('+0000')
