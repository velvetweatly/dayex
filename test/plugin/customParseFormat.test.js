...
dayex.extend(localizedFormats)
dayex.extend(weekOfYear) // test parse w, ww
...
it('does not break the built-in parsing', () => {
  ...
  expect(dayex(input).valueOf()).toBe(moment(input).valueOf())
})
...
it('parse string for MMM month format', () => {
  ...
  expect(dayex(input2, format2).valueOf()).toBe(moment(input2, format2).valueOf())
})
  ...
  ['zh-cn', 'ru', 'uk', 'en'].forEach((lo) => {
    it(`Locale: ${lo}`, () => {
      ...
      longDateFormats.forEach((f) => {
        ...
        expect(dayex(localizedInput, f).valueOf()).toBe(moment(localizedInput, f).valueOf())
      })
    })
  })
})
...
describe('Timezone Offset', () => {
  it('timezone with 2-digit offset', () => {
    ...
    const result = dayex(input, format)
...
it('fails with an invalid format', () => {
  const input = '2018-05-02 12:00 PM'
  const format = 'C'
  expect(dayex(input, format).format().toLowerCase())
...
it('parse month from string with locale in config', () => {
  ...
  expect(dayex(input, format, 'uk').valueOf()).toBe(moment(input, format, 'uk').valueOf())
})
...
it('return Invalid Date when parse corrupt string', () => {
  ...
  const format = 'YYYY MMMM DD'
  expect(dayex(input, format).format()).toBe('Invalid Date')
})
...
it('YYYY-MM set 1st day of the month', () => {
  expect(dayex('2019-02', 'YYYY-MM').format('YYYY-MM-DD')).toBe('2019-02-01')
})

it('Invalid Dates', () => {
  expect(dayex('10/12/2014', 'YYYY-MM-DD').format('MM-DD-YYYY')).toBe('Invalid Date')
  expect(dayex('10-12-2014', 'YYYY-MM-DD').format('MM-DD-YYYY')).toBe('Invalid Date')
})

it('Valid Date', () => {
  expect(dayex('2014/10/12', 'YYYY-MM-DD').format('MM-DD-YYYY')).toBe('10-12-2014')
})
...
it('correctly parse ordinal', () => {
  ...
  expect(dayex(input, format).valueOf())
    .toBe(moment(input, format).valueOf())
  expect(dayex(input2, format).valueOf())
...
describe('month function locale', () => {
  it('MMMM', () => {
    ...
    expect(dayex(input, format, 'ru').valueOf()).toBe(moment(input, format, 'ru').valueOf())
    expect(dayex(input2, format, 'ru').valueOf()).toBe(moment(input2, format, 'ru').valueOf())
  })
  it('MMM', () => {
...
describe('Strict mode', () => {
  it('without locale', () => {
    ...
    expect(dayex(input, format).isValid()).toBe(true)
    expect(dayex(input, format, true).isValid()).toBe(false)
    expect(dayex('2020-Jan-01', 'YYYY-MMM-DD', true).isValid()).toBe(true)
    expect(dayex('30/1/2020 10:59 PM', 'D/M/YYYY h:mm A', true).isValid()).toBe(true)
  })
  it('with locale', () => {
    const input = '2018 三月 99'
    const format = 'YYYY MMMM DD'
    expect(dayex(input, format, 'zh-cn').isValid()).toBe(true)
    expect(dayex(input, format, 'zh-cn', true).isValid()).toBe(false)
  })
})
...
describe('Array format support', () => {
  it('second ok', () => {
    ...
    expect(dayex(input, format).isValid()).toBe(true)
    expect(dayex(input, format, true).format('YYYY-MM-DD')).toBe('2012-05-28')
  })
  it('all invalid', () => {
    const input = '2012-05-28'
  ...
  it('with locale', () => {
    ...
    expect(dayex(input, format, 'zh-cn', true).format('YYYY MMMM DD')).toBe(input)
  })
})
...
describe('meridiem locale', () => {
  ...
  it('AM', () => {
    ...
    const date = dayex(input).locale('zh-cn').format(format)
    expect(dayex(date, format, 'zh-cn').format(format2)).toBe(input)
  })
  it('PM', () => {
...
it('custom two-digit year parse function', () => {
  ...
  expect(dayex(input, format).year()).toBe(1800)
  const input2 = '50-05-02'
  expect(dayex(input2, format).year()).toBe(1850)
  const input3 = '99-05-02'
  expect(dayex(input3, format).year()).toBe(1899)
})
...
describe('parse with special separator characters', () => {
  ...
  it('parse comma separated date in strict mode', () => {
    const input = '20,11,2022'
    const format = 'DD,MM,YYYY'
    const resultDayex = dayex(input, format, true)
...
it('parse X x', () => {
  ...
  expect(dayex(input2, format2, true).valueOf()).toBe(moment(input2, format2, true).valueOf())
})
...
it('parse Q, [Q]', () => {
  ...
  expect(dayex(input1, format).valueOf()).toBe(moment(input1, format).valueOf())
  expect(dayex(input2, format).valueOf()).toBe(moment(input2, format).valueOf())
  expect(dayex(input3, format).valueOf()).toBe(moment(input3, format).valueOf())
  expect(dayex(input4, format).valueOf()).toBe(moment(input4, format).valueOf())
})

it('parse w, ww', () => {
  const input = '2024-w1'
  const format1 = 'YYYY-[w]w'
  expect(dayex(input, format1).format(format1)).toBe(input)
  const input2 = '2024-w32'
  const format2 = 'YYYY-[w]ww'
  expect(dayex(input2, format2).format(format1)).toBe(input2)
})
