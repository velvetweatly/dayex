...
describe('Guess', () => {
  it('return string', () => {
    expect(typeof dayex.tz.guess()).toBe('string')
  })
})
...
describe('Parse', () => {
  it('parse target time string', () => {
    const newYork = dayex.tz('2014-06-01 12:00', NY)
...
describe('Convert', () => {
  it('convert to target time', () => {
    const losAngeles = dayex('2014-06-01T12:00:00Z').tz('America/Los_Angeles')
  ...
  it('convert from time with timezone to target time', () => {
    const losAngelesInUTC = dayex('2014-06-01T05:00:00-07:00').tz('UTC')
...
describe('DST, a time that never existed Fall Back', () => {
  ...
  // and dayex result it as "2012-11-04T01:59:00-05:00"
...
describe('set Default', () => {
  it('default timezone', () => {
    ...
    dayex.tz.setDefault(NY)
    const newYork = dayex.tz(dateStr)
    expect(newYork.format()).toBe('2014-06-01T12:00:00-04:00')
    expect(newYork.utcOffset()).toBe(-240)
    expect(newYork.valueOf()).toBe(1401638400000)

    expect(dayex(dateStr).tz().format()).toBe(dayex(dateStr).tz(NY).format())
  })

  it('empty timezone means local timezone', () => {
    const LOCAL_TZ = dayex.tz.guess()
    const dateStr = '2014-06-01 12:00'
    dayex.tz.setDefault()
    expect(dayex(dateStr).tz().valueOf()).toBe(dayex(dateStr).tz(LOCAL_TZ).valueOf())
    expect(dayex.tz(dateStr).valueOf()).toBe(dayex.tz(dateStr, LOCAL_TZ).valueOf())
  })

  it('change default timezone', () => {
    dayex.tz.setDefault(NY)
    const newYork = dayex.tz('2014-06-01 12:00')
    expect(newYork.utcOffset()).toBe(-240)

    dayex.tz.setDefault(TOKYO)
    const tokyo = dayex.tz('2014-06-01 12:00')
  ...
  it('override default timezone in proto.tz', () => {
    dayex.tz.setDefault(NY)
    const tokyo = dayex.tz('2014-06-01 12:00', TOKYO)
    expect(tokyo.format()).toBe('2014-06-01T12:00:00+09:00')
    expect(tokyo.format('Z')).toBe('+09:00')
    expect(tokyo.valueOf()).toBe(1401591600000)
  })
...
describe('keepLocalTime', () => {
  const base = dayex.tz('2013-11-18 11:55', 'America/Toronto')
  it('keepLocalTime', () => {
  ...
  it('keeps local time when converting between DST-desynced zones', () => {
    const d = dayex.tz('2024-03-20 12:00', NY).tz(PARIS, true)
...
describe('Get offsetName', () => {
  const dtz = dayex.tz('2012-03-11 01:59:59', NY)
  it('short', () => {
...
describe('CustomPraseFormat', () => {
  ...
  it('normal', () => {
    expect(dayex.tz('2020/10/15 12:30', DEN).unix()).toBe(result)
  })
  it('custom', () => {
    expect(dayex.tz('10/15/2020 12:30', 'MM/DD/YYYY HH:mm', DEN).unix()).toBe(result)
  })
})

describe('startOf and endOf', () => {
  it('corrects for timezone offset in startOf', () => {
    const originalDay = dayex.tz('2010-01-01 00:00:00', NY)
  ...
  it('corrects for timezone offset in endOf', () => {
    const originalDay = dayex.tz('2009-12-31 23:59:59.999', NY)
  ...
  it('preserves locality when tz is called', () => {
    const tzWithoutLocality = dayex.tz('2023-02-17 00:00:00', NY)
    const tzWithLocality = dayex.tz('2023-02-17 00:00:00', NY).locale({
      name: 'locale_test',
      weekStart: 3
    })
...
describe('UTC timezone', () => {
  it('TZ with UTC with Locale', () => {
    const test1 = dayex('2000-01-01T09:00:00+09:00').tz('Asia/Seoul').locale('en')
    expect(test1.hour()).toBe(9)
    const test2 = dayex('2000-01-01T09:00:00+09:00').tz('Asia/Hong_Kong').locale('en')
    expect(test2.hour()).toBe(8)
    const test3 = dayex('2000-01-01T09:00:00+09:00').tz('Etc/UTC').locale('en')
...
// https://github.com/velvetweatly/dayex/issues/1260
describe('Fixed-offset zone across host DST (America/Guadeloupe)', () => {
  ...
  it('keeps UTC−4 when converting an instant around host spring DST', () => {
    const d = dayex(instant).tz(GUADELOUPE)
    expect(d.utcOffset()).toBe(-240)
  ...
  it('advances the calendar day when adding 1 day across host spring DST', () => {
    ...
    const days = []
    for (let i = 0; i < 3; i += 1) {
      const cur = dayex(date).tz(GUADELOUPE)
      days.push({
...
describe('DST-observing zone across host DST', () => {
  ...
  it('converts to America/New_York while US is in DST and EU is not', () => {
    const d = dayex(springDesync).tz(NY)
    expect(d.utcOffset()).toBe(-240)
  ...
  it('converts to Europe/Paris while EU is still on standard time', () => {
    const d = dayex(springDesync).tz(PARIS)
    expect(d.utcOffset()).toBe(60)
  ...
  it('converts to America/New_York while US is still in DST and EU has fallen back', () => {
    const d = dayex(fallDesync).tz(NY)
    expect(d.utcOffset()).toBe(-240)
  ...
  it('converts to Europe/Paris after EU has fallen back', () => {
    const d = dayex(fallDesync).tz(PARIS)
    expect(d.utcOffset()).toBe(60)
  ...
  it('converts to Australia/Sydney while southern hemisphere is in DST', () => {
    const d = dayex(southernSummer).tz(SYDNEY)
  ...
  it('converts to Europe/Paris while northern hemisphere is on standard time', () => {
    const d = dayex(southernSummer).tz(PARIS)
    expect(d.utcOffset()).toBe(60)
...
describe('DST edge cases vs moment', () => {
  it('parses the fall-back overlap the same way as moment', () => {
    ['01:00', '01:30', '01:59'].forEach((time) => {
      ...
      matchMoment(dayex.tz(s, NY), moment.tz(s, NY))
    })
  })

  it('keeps the two 01:30 instants distinct when converting', () => {
    matchMoment(
      dayex('2012-11-04T05:30:00.000Z').tz(NY),
      moment('2012-11-04T05:30:00.000Z').tz(NY)
    )
    matchMoment(
      dayex('2012-11-04T06:30:00.000Z').tz(NY),
  ...
  it('converts through the host fall-back overlap', () => {
    ...
    matchMoment(dayex(iso).tz(GUADELOUPE), moment(iso).tz(GUADELOUPE))
  })

  it('converts 30-minute DST (Australia/Lord_Howe)', () => {
    matchMoment(
      dayex('2024-01-15T12:00:00.000Z').tz('Australia/Lord_Howe'),
      moment('2024-01-15T12:00:00.000Z').tz('Australia/Lord_Howe')
    )
    matchMoment(
      dayex('2024-07-15T12:00:00.000Z').tz('Australia/Lord_Howe'),
  ...
  it('converts Europe/London in winter (UTC+0)', () => {
    matchMoment(
      dayex('2024-01-15T12:00:00.000Z').tz(LONDON),
      moment('2024-01-15T12:00:00.000Z').tz(LONDON)
    )
    matchMoment(
      dayex.tz('2024-01-15 12:00', LONDON),
  ...
  it('handles the 30-minute Lord Howe gap and both sides of its overlap', () => {
    ...
    matchMoment(
      dayex.tz('2024-10-06 02:15', zone),
    ...
    overlapInstants.forEach((instant) => {
      matchMoment(dayex(instant).tz(zone), moment(instant).tz(zone))
    })
  })
  ...
  it('keeps local time when converting a host-local instance', () => {
    ...
    matchMoment(
      dayex(input).tz(GUADELOUPE, true),
      moment(input).tz(GUADELOUPE, true)
    )
  })

  it('diffs elapsed hours and calendar days across target DST', () => {
    const dBefore = dayex.tz('2012-03-10 12:00', NY)
    const dAfter = dayex.tz('2012-03-11 12:00', NY)
  ...
  it('handles quarter-hour and date-line offsets', () => {
    ...
    zones.forEach((zone) => {
      matchMoment(dayex(instant).tz(zone), moment(instant).tz(zone))
      matchMoment(
        dayex.tz('2024-01-01 00:15', zone),
...
it('does not throw for invalid string in d.tz()', () => {
  expect(() => dayex.tz('invalid', 'Europe/Skopje')).not.toThrow()
  expect(dayex.tz('invalid', 'Europe/Skopje').isValid()).toBe(false)
})
