...
it('Constructor from Object', () => {
  for (let i = 0; i < tests.length; i += 1) {
    expect(dayex(tests[i][0]).format(fmt)).toBe(tests[i][1])
...
it('Constructor from Object UTC', () => {
  for (let i = 0; i < tests.length; i += 1) {
    ...
    expect(dayex.utc(tests[i][0]).format(fmt)).toBe(result)
    expect(moment.utc(tests[i][0]).format(fmt)).toBe(result)
  }
})

it('Constructor from null should return Invalid Date', () => {
  expect(dayex(null).isValid()).toBe(false)
  expect(moment(null).isValid()).toBe(false)
})

it('Set from Object', () => {
  for (let i = 0; i < tests.length; i += 1) {
    expect(dayex(now).set(tests[i][0]).format(fmt)).toBe(moment(now).set(tests[i][0]).format(fmt))
  }
})
...
it('add short reverse args', () => {
  ...
  const b = dayex([2010, 1, 31]).add({ M: 1 })
  const c = dayex([2010, 2, 28]).subtract({ M: 1 })
  const d = dayex([2010, 2, 28]).subtract({ Q: 1 })
...
it('add decimal values of days and months', () => {
  expect(dayex([2016, 4, 3]).add(1.6, 'days').date()).toBe(5)
  expect(dayex([2016, 4, 3]).add(-1.6, 'days').date()).toBe(1)
  expect(dayex([2016, 4, 1]).add(-1.6, 'days').date()).toBe(30)
  expect(dayex([2016, 4, 3]).add(1.6, 'months').month()).toBe(4)
  expect(dayex([2016, 4, 3]).add(-1.6, 'months').month()).toBe(1)
  expect(dayex([2016, 1, 3]).add(-1.6, 'months').month()).toBe(11)
  expect(dayex([2016, 4, 3]).subtract(1.6, 'days').date()).toBe(1)
  expect(dayex([2016, 4, 2]).subtract(1.6, 'days').date()).toBe(31)
  expect(dayex([2016, 2, 1]).subtract(1.1, 'days').date()).toBe(31)
  expect(dayex([2016, 4, 3]).subtract(-1.6, 'days').date()).toBe(5)
  expect(dayex([2016, 4, 30]).subtract(-1.6, 'days').date()).toBe(2)
  expect(dayex([2016, 4, 3]).subtract(1.6, 'months').month()).toBe(1)
  expect(dayex([2016, 4, 3]).subtract(-1.6, 'months').month()).toBe(4)
  expect(dayex([2016, 12, 31]).subtract(-1.6, 'months').month()).toBe(0)
  expect(dayex([2016, 1, 1]).add(1.6, 'years').format('YYYY-MM-DD')).toBe('2017-01-01')
  expect(dayex([2016, 7, 1]).add(1.6, 'years').format('YYYY-MM-DD')).toBe('2017-07-01')
  expect(dayex([2016, 1, 1]).add(1.1, 'quarters').format('YYYY-MM-DD')).toBe('2016-04-01')
})

it('returns valid date on undefined', () => {
  expect(dayex().isValid()).toBe(true)
})
